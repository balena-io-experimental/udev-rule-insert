...
cat batch | stdbuf -oL xargs -I{} -P 30 /bin/sh -c "grep -a -q '{} : DONE' udev-rules.log || (cat add-udev-rule.sh | balena ssh {} -s --noninteractive | sed 's/^/{} : /' | tee --append udev-rules.log)"
